...
namespace Core
{
	...
	void ObjectFuncParser::Parse(const string functionStr, string fileName)
	{
	...
	void FunctionParser::Parse(const string functionStr, string fileName)
	{
		//return nullptr;
	}
	...
	void GradPasser::Parse(const string functionStr, const string fileName, int index,  string& className)
	{
		...
		bool cppState = WriteCPPFile(functionStr, fileName, index, className);
	...
	bool GradPasser::WriteCPPFile(const string gradStr, const string fileName, int index, string& className)
	{
		vector<string> lines;
	...
	void ExcuteGradParser::Parse( const string fileName, const vector<string> vecClassNames)
	{
	...
	void ExcuteGradParser::ParseInequalGrad(const string fileName, const vector<string> vecClassNames)
	{
		vector<string> lines;
		ifstream in(fileName);
		...
		//Find the position to insert function. The insert does not use this as benchmark, it insert right
		//above the mark of "insert end"
		string startMark = "//Compile Inequal Grad -insert start";
		auto insertPos = find(lines.begin(), lines.end(), startMark);
		//

		string endMark = "//Compile Inequal Grad -insert end";
		//Find the position to stop. The insert start from right above this mark.
		...
		for (size_t i = 0; i < vecClassNames.size(); i++)
		{
			...
			//string insertCode2 = "ObjFuncExcut::PushGradPnt(" + objectName + ");";
			string insertCode2 = "_temGrad.push_back(" + objectName + ");";
	...
	InequalConstrtFunParser::InequalConstrtFunParser()
	{
		InitRccSystem();
		_eraseExistingCodes_h=true;
		_eraseExistingCodes_cpp= true;
	...
	void InequalConstrtFunParser::Parse(const string functionStr, const string fileName, int index, string & className)
	{
		string indexStr = std::to_string(index);
		className = "InequalCnstrntFunc" + indexStr;
	...
	IFunction * InequalConstrtFunParser::DynamicCompile(const char* fileName)
	{
		//Log::PushNDC("InequalConstrtFunParser");
		...
		//Get object function from factory.
		auto *Ptr = _sys->GetObjectFactorySystem()->GetConstructor("InequalCnstrntFunc")->Construct();
		return dynamic_cast<InequalCnstrntFunc*>(Ptr);
		
	}
	void InequalConstrtFunParser::InitRccSystem()
	{
		StdioLogSystem *pThreadsafeLog = new StdioLogSystem();
	...
	bool InequalConstrtFunParser::WriteHeadlerFile(const string gradStr, const string fileName, int index, string& className)
	{
		vector<string> lines;
		ifstream in(fileName + ".h");
		...
		//Find the position to insert function.
		string startMark = "//Inequal Constraint Function Class Headler file-insert start";
		auto insertPos = find(lines.begin(), lines.end(), startMark);

		//Find the position to stop
		string endMark = "//Inequal Constraint Function Class Headler file-insert end";
		auto endPos = find(lines.begin(), lines.end(), endMark);
		...
		vector<string> newCodes;
		string newCode = "class\t" + className + ": public InequalCnstrntFunc";
		newCodes.push_back(newCode);
		newCode = "{";
		newCodes.push_back(newCode);
		newCode = "public:";
	...
	bool InequalConstrtFunParser::WriteCPPFile(const string gradStr, const string fileName, int index, string& className)
	{
		vector<string> lines;
		ifstream in(fileName + ".cpp");
		...
		//Find the position to insert function.
		string startMark = "//Inequal Cnst Class CPP file-insert start";
		auto insertPos = find(lines.begin(), lines.end(), startMark);

		//Find the position to stop
		string endMark = "//Inequal Cnst Class CPP file-insert end";
		auto endPos = find(lines.begin(), lines.end(), endMark);
		...
		newCode = "{";
		newCodes.push_back(newCode);
		newCode = "double inequalCnst=" + gradStr + ";";
		newCodes.push_back(newCode);
		newCode = "return inequalCnst;";
		newCodes.push_back(newCode);
		newCode = "}";
		newCodes.push_back(newCode);
		//////Register this class for dynamic compiling
	...
	ExcuteInequalConstrtParser::ExcuteInequalConstrtParser()
	{
		InitRccSystem();
	}
	ExcuteInequalConstrtParser::~ExcuteInequalConstrtParser()
	...
	void ExcuteInequalConstrtParser::Parse(const string fileName, const vector<string> vecClassNames)
	{
		vector<string> lines;
		ifstream in(fileName);
		...
		//Find the position to insert function. The insert does not use this as benchmark, it insert right
		//above the mark of "insert end"
		string startMark = "//CompileInequalCnstFunc-insert start";
		auto insertPos = find(lines.begin(), lines.end(), startMark);
		//

		string endMark = "//CompileInequalCnstFunc-insert end";
		//Find the position to stop. The insert start from right above this mark.
		auto endPos = find(lines.begin(), lines.end(), endMark);
		...
		for (size_t i = 0; i < vecClassNames.size(); i++)
		{
			...
			string insertCode1 = vecClassNames[i] + "* ";
			string objectName = "inequal" + std::to_string(int(i));
			...
			//string insertCode2 = "ObjFuncExcut::PushGradPnt(" + objectName + ");";
			string insertCode2 = "_ineqFuncDefin.push_back(" + objectName + ");";
	...
	IFunction * ExcuteInequalConstrtParser::DynamicCompile(const char * fileName)
	{
		Log::PushNDC("ExcuteInequalConstrtParser");
		...
		//Get object function from factory.
		auto *CompileGradPtr = _sys->GetObjectFactorySystem()->GetConstructor("CompileIneqlCnstFuncExcut")->Construct();

		//auto *objFunc = _sys->GetObjectFactorySystem()->GetConstructor(fileName)->Construct();		
		return dynamic_cast<CompileIneqlCnstFuncExcut*>(CompileGradPtr);
	}
	void ExcuteInequalConstrtParser::InitRccSystem()
	{
		StdioLogSystem *pThreadsafeLog = new StdioLogSystem();
